...
for($i=0;$i<$num;++$i){ 
	...
	my $sth = $dbh->prepare('SELECT name, source_database,db_accession FROM gn2pathway where UPPER(symbol)=? and name is not null');
...
while ((my$pathway, my$pathsource) = each(%pathwayname)){
	...
	my $sth = $dbh->prepare('SELECT   symbol,name, source_database,db_accession FROM gn2pathway where name=?  group by symbol,name,source_database,db_accession');
